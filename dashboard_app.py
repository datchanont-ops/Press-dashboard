...
# ==========================================
# ⚙️ Sidebar: Google Drive & Local Save Path
# ==========================================
with st.sidebar:
...
with col_upload:
    ...
    # --- ระบบอัปโหลดและเซฟไฟล์ลง Local Path ---
    active_db_file = None
    if db_file is not None:
        active_db_file = db_file
        if st.button("💾 บันทึกไฟล์นี้ลง Google Drive", use_container_width=True):
            file_bytes = bytes(db_file.getbuffer())
            with open(SAVED_DB_PATH, "wb") as f:
                f.write(file_bytes)
            st.success(f"✅ บันทึกไฟล์เรียบร้อย! (ระบบจะ Sync ขึ้น Cloud ให้เอง)")
            
    elif os.path.exists(SAVED_DB_PATH):
        active_db_file = SAVED_DB_PATH
        st.caption(f"📌 กำลังแสดงผลจากไฟล์ที่บันทึกไว้ใน:\n`{save_path}`")
