...
if active_db_file:
    with st.spinner("🔄 กำลังประมวลผลข้อมูล (Processing Data)..."):
        ...
        if search_query:
            ...
            if queries:
                ...
                if not searched_df.empty:
                    # ดักจับ Error หากผู้ใช้รันบน Pandas เวอร์ชันเก่า (ซึ่งไม่มี Styler.map)
                    try:
                        ...
                        styled_search = searched_df.style.applymap(color_balance, subset=[balance_col_name]).applymap(color_wip, subset=['WIP Day']).applymap(color_bo_date, subset=['B/O Date']).format(format_dict)
        ...
        with col_table:
            ...
            # จัดการสีแบบ Clean & Corporate (ดักจับ Error สำหรับ Pandas เวอร์ชันเก่า)
            try:
                ...
                styled_df = df_short.style.applymap(color_balance, subset=[balance_col_name])\
                                          ...
                                          .applymap(lambda x: 'color: #3b82f6; font-weight: 600;', subset=['Part No.', 'Material'])\
                                          .applymap(lambda x: 'color: #ef4444; font-weight: 600;' if x > 0 else '', subset=['Orders'])\
                                          .format(format_dict)
